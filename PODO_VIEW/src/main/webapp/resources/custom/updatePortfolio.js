...
(success)=>{
	$("#portfolio_title").val(success['title']);
	$("#portfolio_header").val(success['header']);
	$("#summernote").summernote('code',success['content']);
	$("#showType").val(success['showType']).prop("selected", true);
	$("#title_text_length").text("- 100 자 중 ( "+ success['title'].length + " ) 자 작성");
	$("#header_text_length").text("- 100 자 중 ( "+ success['header'].length + " ) 자 작성");
...
$("#updateBtn").click(()=>{
	...
	portfolioUpdate(portfolio,(success)=>{
		$("#portfolioUpdateSuccessModal").modal();
	},(error)=>{
...
$("#portfolioUpdateSuccessBtn").click(()=>{
	location.href = './portfolioDetail?seq=' + seq;
});
...
//		data : data,
//		type : "POST",
//		url : "http://localhost:8082/api/v1/portfolio/fileUpload",
//		contentType : false,
//		processData : false,
//		success : function(data) {
//        	//항상 업로드된 파일의 url이 있어야 한다.
//			$(editor).summernote('insertImage', "http://localhost:8082/" + data);
//		}
//	});
//}